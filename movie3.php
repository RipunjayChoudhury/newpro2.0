...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['movie_id']) && isset($_POST['seats_booked'])) {
        ...
        $stmt = $con->prepare("INSERT INTO mbookings (user_email, id, hall_id, location_id, seat_no, price, status) VALUES (?, ?, ?, ?, ?, ?, 'Pending')");
        $stmt->bind_param("siiiii", $userData['email'], $selected_movie, $selected_hall, $selected_location, $seats_booked, $price);
        $stmt->execute();
